...
public class FollowersStatsImpl implements FollowersStats {
  ...
  public Future<Integer> followersCountBy(int id, int depth, Predicate<UserInfo> predicate) {
    return followersCountBy(id, depth, predicate, new ConcurrentSkipListSet<>() {{ add(id); }});
  }

  private CompletableFuture<Integer> followersCountBy(int id, int depth,
      Predicate<UserInfo> predicate, ConcurrentSkipListSet<Integer> visited) {
    CompletableFuture<Integer> userInfo = network.getUserInfo(id).thenApply(user -> predicate.test(user) ? 1 : 0);
